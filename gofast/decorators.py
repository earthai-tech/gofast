...
class temp2d: 
    ...
    :class:`gofast.property.BasePlot` parameters. If not given, an 
    ...
    def __getattr__(self, name): 
        msg = ("{0!r} has no attribute {1!r}. Note that {0!r} uses the"
               " plot arguments from `gofast.property.BasePlot`. Plot arguments"
...
class refAppender (object): 
    ...
    Examples
    ---------
    >>> from gofast.documentation import __doc__ 
    >>> from gofast.tools import decorators
    >>> def donothing (): 
                ...
                if self.to =='.csv': 
                    ...
                    self.df.to_excel(self.refout , sheet_name='{0}'.format(
                        self.refout[: int(len(self.refout)/2)]),
                            index=self.writedfIndex) 
            # savepath 
            generatedfile = '_gofast{}_'.format(
...
class docstring:
    ...
    >>> from gofast.decorators import writedf , predPlot, docstring 
...
class docAppender: 
    ...
    Examples
    ---------
    >>> from gofast.decorators import docAppender 
...
class docSanitizer: 
    ...
    Examples
    --------
    >>> from gofast.decorators import docSanitizer 
    >>> def messdocfunc(): 
