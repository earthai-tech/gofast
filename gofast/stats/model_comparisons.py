...
def plot_model_summary(
    model_performance_data: Union[Dict[str, List[float]], DataFrame], 
    summary_obj: Optional[DataFrameFormatter] = None,    
    higher_is_better: bool = True,
    plot_type: str = 'bar',  
    figsize: Tuple[int, int] = (10, 6)  
):
    """
    Visualizes the summary of model performance data.

    This function supports visualization of model performance using different 
    plot types. It can work with either a `summary_obj` generated by 
    `compute_model_summary` function or directly with `model_performance_data`.
    Supported plot types include bar plots (both vertical and horizontal) and 
    line plots.

    Parameters
    ----------
    model_performance_data : Union[Dict[str, List[float]], DataFrame], optional
        A dictionary or pandas DataFrame containing the performance metrics for
        each model. Each key (for dict) or column (for DataFrame) represents a
        different model, and each value or row corresponds to the model's 
        performance metric on a different dataset. If provided, this is used to
        compute the summary statistics for plotting.
    summary_obj : Optional[DataFrameFormatter], optional
        An instance of DataFrameFormatter containing the formatted summary 
        statistics of model performance. If provided, `model_performance_data`
        is ignored.
    higher_is_better : bool, optional
        Indicates whether higher values of the performance metric represent 
        better model performance. Defaults to True.
    plot_type : str, optional
        The type of plot to generate. Options are 'bar', 'line', 'barh' for 
        vertical bar plots, line plots, and horizontal bar plots, respectively.
        Defaults to 'bar'.
    figsize : Tuple[int, int], optional
        The size of the figure to create. Defaults to (10, 6).

    Raises
    ------
    ValueError
        If both `summary_obj` and `model_performance_data` are None, or if the
        `summary_data` cannot be found in `summary_obj`.
    TypeError
        If `summary_obj` is not an instance of DataFrameFormatter.
    AttributeError
        If `summary_obj` does not have either a 'summary' or 'df' attribute.

    Examples
    --------
    >>> plot_model_summary(model_performance_data={
    ...     'Model A': [0.8, 0.82, 0.85],
    ...     'Model B': [0.75, 0.78, 0.8]},
    ...     plot_type='bar')
    This will plot vertical bars showing the performance of Model A and Model B.

    >>> plot_model_summary(model_performance_data=pd.DataFrame({
    ...     'Model A': [0.8, 0.82, 0.85],
    ...     'Model B': [0.75, 0.78, 0.8]}),
    ...     plot_type='line')
    This will plot a line graph showing the performance of Model A and Model B.

    Notes
    -----
    - The function is designed to provide flexibility in visualizing model 
      performance summaries. It supports input either directly as performance 
      data or as a precomputed summary object.
    - The `DataFrameFormatter` is assumed to be a custom class for formatting 
      pandas DataFrames. Ensure it has either a 'summary' or 'df' attribute 
      containing summary statistics for the plot.
    - The figure size and plot type can be customized to suit different 
      visualization needs.

    """
    if summary_obj is None and model_performance_data is None:
        raise ValueError("Either summary_obj or model_performance_data must be provided.")

    if model_performance_data is not None:
        summary_obj = compute_model_summary(model_performance_data, higher_is_better)

    if not isinstance(summary_obj, DataFrameFormatter):
        raise TypeError("summary_obj must be an instance of DataFrameFormatter.")
    
    if not hasattr(summary_obj, 'summary') and not hasattr(summary_obj, 'df'):
        raise AttributeError("summary_obj must have either a 'summary' or 'df' attribute.")
    
    if hasattr ( summary_obj, 'summary'): 
        summary_data = getattr(summary_obj, 'summary', None)
    else : summary_data=getattr(summary_obj, 'df', None)
    
    if summary_data is None:
        raise ValueError("Summary data could not be found in summary_obj.")


    # Check the plot_type and apply appropriate plotting
    plot_type = str(plot_type).lower() 
    if plot_type not in ['bar', 'line', 'barh']:
        raise ValueError(f"Unsupported plot_type: {plot_type}. Choose"
                         " from 'bar', 'line', 'barh'.")

    if plot_type == 'line':
        summary_data.T.plot(kind='line', marker='o', linestyle='-',
                          color='skyblue', legend=False)
    else:
        summary_data.T.plot(kind=plot_type, color='skyblue', legend=False)

    plt.title('Model Performance Summary')
    plt.ylabel('Performance')
    plt.xticks(rotation=45, ha='right' if plot_type != 'barh' else 'center')
    plt.tight_layout()
